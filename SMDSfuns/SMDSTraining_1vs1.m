function [model,cutpoint]=SMDSTraining_1vs1(data_train, target_train,...
...
for i=1:T-1
    for j=i+1:T
        model(t).a=i;
        model(t).b=j;
        lable_a = ClassLabels(i);
        model(t).lable_a = lable_a;
        lable_b = ClassLabels(j);
        ...
        fprintf(fid,'--------------------------------------------------------------\n');
        g =(pars.PY==lable_a|pars.PY==lable_b);
        D_2c=D(g,g);
        target_train_2c= pars.PY(:,g)';
        n_2c = size(D_2c,1);  
        ...
        cutpoint(t)=SQREDM_SMDS_test_cutpoint(model(t),D_2c);
        t=t+1;
    end
end
