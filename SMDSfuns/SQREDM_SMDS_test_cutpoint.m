function cutpoint=SQREDM_SMDS_test_cutpoint(model,D)
%SQREDM_SMDS_test_cutpoint This code produces the cutpoints for comparison
%between two objective function from two classification assumption.
%
%
%
%
...
%% 
    ...
    for t=1:T
        ...
        F_crit_1s   = zeros(1,size(D_te,1));
        F_crit_2s   = zeros(1,size(D_te,1));
        X_new_1s    = zeros(size(Out.X,1),size(D_te,1));
        X_new_2s    = zeros(size(Out.X,1),size(D_te,1));
        a           = model(t).lable_a;
        b           = model(t).lable_b;
        for i = 1:size(D_te,1)
            ...
            F_crit_1s(i)   = cf.F_crit_1;
            F_crit_2s(i)   = cf.F_crit_2;
            X_new_1s(:,i)  = cf.X_new_1;                                                                                                    
            X_new_2s(:,i)  = cf.X_new_2;
        end
        F_crit_s = F_crit_1s-F_crit_2s;
%         F_crit_sort = sort(F_crit_s);
        cutpoint(t)    = prctile(F_crit_s,(length(find(pars.PY==a))/length(F_crit_1s))*100);
%         cutpoint(t) = F_crit_sort(length(find(pars.PY==a)));
