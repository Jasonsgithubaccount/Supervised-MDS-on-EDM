function [errorrate, model, Out_te, data_train_ori, target_train_ori, data_test_ori, target_test_ori,std_acc] = smds_main(filename_train, dim, alpha, q, disttype, cla_method,varargin)
...
%                     end
        ...
        Accuracy = Out_te.Accuracy;   
        errorrate = 1 - Accuracy;      std_acc = std(Accuracy);  
end

B = '';        
if ismember('bootstrap',varargin) && ~ismember('distance',varargin)
    B = str2num(cell2mat(varargin(find(ismember(varargin,'bootstrap'),1)+1)));
%      indices = dlmread(cell2mat(varargin(find(ismember(varargin,'k-fold'),1)+1)));
%      k = max(indices);
        switch cla_method
            ...
            case '1vsall'
                ...
                if ~ismember('extrapar',varargin)
                    ...
                    [ Accuracy, model, Out_te ] = Bootstrap632_SMDS( B, data_train_ori, ...
                        target_train_ori, dim, alpha, q, disttype);
                else
%                     if nargin ==10 && strcmp(varargin(9),'extra')
                        par_extra = cell2struct(varargin(find(ismember(varargin,'extrapar'),1)+1));
                        [ Accuracy, model, Out_te ] = Bootstrap632_SMDS( B, data_train_ori, ...
                            target_train_ori, dim, alpha, q, disttype,  'extrapar',par_extra);
%                     else
%                         error('Some input arguments are not correct.')
%                     end
        ...
        switch cla_method
            ...
            case '1vsall'
                ...
                if ~ismember('extrapar',varargin)
                    ...
                    [ Sol, model, Out_te ] = Bootstrap632_SMDS( B, data_train_ori, ...
                        target_train_ori, dim, alpha, q, disttype, 'distance');
                else
%                     if nargin ==10 && strcmp(varargin(9),'extra')
                        par_extra = cell2struct(varargin(find(ismember(varargin,'extrapar'),1)+1));
                        [ Sol, model, Out_te ] = Bootstrap632_SMDS( B, data_train_ori, ...
                            target_train_ori, dim, alpha, q, disttype, 'distance','extrapar',par_extra);
%                     else
%                         error('Some input arguments are not correct.')
%                     end
...
%                     end
    ...
    target_test_ori  =  target_train_ori;                
    errorrate = Sol;          
    Accuracy = Sol.err_632plus;std_acc = std(Accuracy);
end                       
                
            

             
result_folder = strcat('/scratch/dy1n16/wSMDSEDMResults/',dataname);        
...
% title([dataname,', SMDS:',' ','dim',' ',num2str(dim),...
...
%         '_alpha_',num2str(alpha),'_q_',num2str(q),'_dist_',disttype(1:3),...
%         '_',cla_method,'_',char(varargin(find(ismember(varargin,'test'),1))),...
%         '_',char(varargin(find(ismember(varargin,'k-fold'),1))),num2str(k),...
%         '_',char(varargin(find(ismember(varargin,'bootstrap'),1))),num2str(B),...
%         '_recall_',num2str(mean(Accuracy),...
%         '%1.4f'),'.png'));
%     close;
% end
...
movefile ('Output.txt' , strcat(result_folder, '/', dataname,'_SMDS','_d',num2str(dim),...
    '_al',num2str(alpha),'_q',num2str(q),'_',disttype(1:3),...
    '_',cla_method,'_',char(varargin(find(ismember(varargin,'test'),1))),...
    '_',char(varargin(find(ismember(varargin,'k-fold'),1))),num2str(k),...
    '_',char(varargin(find(ismember(varargin,'bootstrap'),1))),num2str(B),...
    '_re_',num2str(mean(Accuracy),...
    '%1.4f'),'.txt'));


% fid_re = fopen(strcat('Results_extended\', dataname, '\',dataname,'_results', '.txt'),'a+');
fid_re = fopen(strcat(result_folder, '/',dataname,'_results', '.txt'),'a+');
...
fprintf(fid_re,strcat('dim:',num2str(dim),...
    ...
    ',  ',char(varargin(find(ismember(varargin,'k-fold'),1))),': ',num2str(k),...
    ',  ',char(varargin(find(ismember(varargin,'bootstrap'),1))),': ',num2str(B),...
    ', recall:',num2str(mean(Accuracy),...
    '%1.4f'),'\n'));
fclose('all');
...
disttype={'euclidean','squaredeuclidean',...
    ...
    'DDTWR1','DDTWR02','DDTWR05','DDTWR08','LCSS','ERP','MSM','WDTWG08',...
