function Out_te=SQREDM_SMDS_test_1vsall(model,cutpoint)
%This code is 
%% 
...
%end
    ...
    %         tref= tic;
            cf(i)= TestSuperMDSSingleObs (D_te(i,:), alpha, Out, pars.PY,a, b,q);
    %         time= toc(tref); 
    %         fprintf(fid,'Time: %1.3fsec\n',time);
            F_crit_1s(i,t)   = cf(i).F_crit_1;
            F_crit_2s(i,t)   = cf(i).F_crit_2;
            %fprintf(fid,'Item %d: F2-F1: %1.3f; F2/F1: %1.3f; PY:%1d\n', i,F_crit_2s(i)-F_crit_1s(i),F_crit_2s(i)/F_crit_1s(i),Out_te.PY_te(i) );
            X_new_1s(:,i,t)  = cf(i).X_new_1;
            X_new_2s(:,i,t)  = cf(i).X_new_2;
        end
%         parpool close;
    ...
    [~,g_f] = min(Out_te.PY_te_F_crit,[],2);
    ClassLabel_m = ones(n_te,1)*ClassLabels';
    Out_te.PY_te_hat_fi = (ClassLabel_m(sub2ind(size(ClassLabel_m),1:n_te,g_f')))';
...
%     Out_te.PY_te_hat_fi = zeros(size(model(1).D_te,1),1);
%     for i = 1 : size(model(1).D_te,1)
%         %[~,g_hat] = max(Out_te.PY_te_F_crit(i,:));
%         [~,g_hat] = max(Out_te.PY_te_F_crit(i,:));
%         Out_te.PY_te_hat_fi(i) = mode(Out_te.PY_te_hat(i,:),2);
%         %Out_te.PY_te_hat_fi(i) = Out_te.PY_te_hat(i,g_hat);
%     end
% 
%     % Out_te.PY_te_hat_fi = mode(Out_te.PY_te_hat,2);
    ...
    fprintf(fid,'\n--------------------------Testing----------------------------\n');
    fprintf(fid,'   Prediction Result of Each Model (Accuracy Rate: %1.3f)   \n', Out_te.Accuracy);
    fprintf(fid,'--------------------------------------------------------------\n');
    for i = 1:n_te
        fprintf(fid,'Item %d: ',i);
        for t = 1 : T
            fprintf(fid,'%d  ',ClassLabels(t));
...
function [D_te,Out] = extension(D_te,Out)
n_te = size(D_te,1);
