function [err, model, Out_te_loo]= BCV_SMDS_1vsall(  B, data, target, dim, alpha, q, disttype,  varargin )
   ...
   fclose(fid);
    ...
    if  ~ismember('distance',varargin)
        if ismember('extrapar',varargin)
         ...
         [errB, model, Out_te_loo] = LOO_SMDS_1vsall(data_train, target_train, ...
...
function [err, model, Out_te_loo] = LOO_SMDS_1vsall(  data, target, dim, alpha, q, disttype,  varargin )
% - leave-one-out cross validation for Canonical Discriminant Analysis
%
% USAGE: [err,PP] = LOO_SMDS(x,grp,method,verb);
%
...
for omit = 1:n
   ...
   Out_te_loo=SQREDM_SMDS_test_1vsall(model,cutpoint); 
